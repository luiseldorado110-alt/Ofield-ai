(() => {
  ...
  function generatePlan() {
    const L = Number(planEl('planL')?.value) || 0, W = Number(planEl('planW')?.value) || 0;
    const levels = Number(planEl('planLevels')?.value) || 2, rooms = Number(planEl('planRooms')?.value) || 3;
    const baths = Number(planEl('planBaths')?.value) || 2, cars = Number(planEl('planCars')?.value) || 0;
    const needs = (planEl('planNeeds')?.value || '').trim(), msg = planEl('conceptPlanMessage'), out = planEl('conceptPlanResult');
    if (!L || !W) { msg.textContent = 'Ingresa frente y fondo del terreno.'; return; }
    msg.textContent = '✦ Generando distribución conceptual...';
    let floors = ''; for (let i = 1; i <= levels; i++) floors += roomPlan(i, rooms, cars, needs);
    ...
    planEl('printConceptPlan').onclick = () => window.print(); msg.textContent = '✓ Plano conceptual generado.';
  }

  function addPlanTool() {
    const tools = document.querySelector('.tools .grid'), section = document.querySelector('#herramientas');
    if (!tools || !section) return;

    let button = document.getElementById('openConceptPlan');
    if (!button) {
      ...
      button = card.querySelector('#openConceptPlan');
    }
    ...
    if (!button.dataset.planBound) {
      button.dataset.planBound = '1';
      button.addEventListener('click', () => { panel.classList.remove('hidden'); panel.scrollIntoView({behavior:'smooth', block:'center'}); });
  ...
  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', addPlanTool); else addPlanTool();
})();